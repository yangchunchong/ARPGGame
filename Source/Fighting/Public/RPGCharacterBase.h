...
UCLASS()
class FIGHTING_API ARPGCharacterBase : public ACharacter, public IAbilitySystemInterface, public IGenericTeamAgentInterface
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable)
	virtual void SetHealth(float fHealth) const;
	...
	UFUNCTION(BlueprintCallable)
	virtual void SetMana(float fMana) const;
...
protected:
	...
	/** Abilities to grant to this character on creation. These will be activated by tag or event and are not bound to specific inputs */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Abilities)
	TMap<FRPGItemSlot, TSubclassOf<URPGGameplayAbility>> SelfGameplayAbilities;
	...
	/** Apply the startup gameplay abilities and effects */
	void AddStartupLevelUpGameplayAbilitie();

	/** Attempts to remove any startup gameplay abilities */
	void RemoveStartupGameplayAbilities();
	/** Apply the startup gameplay abilities and effects */
	void RemoveStartupLevelUpGameplayAbilitie();
