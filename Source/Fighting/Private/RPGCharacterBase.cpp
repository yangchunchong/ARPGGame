...
void ARPGCharacterBase::AddStartupLevelUpGameplayAbilitie()
{
	check(AbilitySystemComponent);
	...
	if (GetLocalRole() == ROLE_Authority)
	{
		...
		for (const TPair<FRPGItemSlot, TSubclassOf<URPGGameplayAbility>>& DefaultPair : SelfGameplayAbilities)
		{
...
void ARPGCharacterBase::RemoveStartupLevelUpGameplayAbilitie()
{
...
void ARPGCharacterBase::RemoveSlottedGameplayAbilities(bool bRemoveAll)
{
	...
	{
		if (ExistingPair.Key.ItemType == URPGAssetManager::CharacterType)
			continue;
...
void ARPGCharacterBase::PossessedBy(AController* NewController)
{
	...
	if (AbilitySystemComponent)
	{
		...
		AddStartupLevelUpGameplayAbilitie();
	}
}
...
void ARPGCharacterBase::SetHealth(float fHealth) const
{
	if (!AttributeSet)
		return ;

	return AttributeSet->SetHealth(fHealth);
}
...
void ARPGCharacterBase::SetMana(float fMana) const
{
	if (!AttributeSet)
		return;

	return AttributeSet->SetMana(fMana);
}
...
void ARPGCharacterBase::CancelAllAbilities()
{
	if (AbilitySystemComponent)
	{
